        ...
        <form class="main__filter__top">
            ...
            <div class="param param_nth js-param" v-for="attr in getAttributesByType('select')" :key="attr.id">
                ...
                <template v-if="attr.items.length > 5">
                    <button class="more js-more" type="button">показать еще <span class="js-not_visible">0</span>
                    </button>
    ...
    methods: {
        ...
        initAttributes() {
            if (Object.keys(this.filter).length === 0) {
                Object.keys(true.getAttributes).forEach((attr) => {
                    this.filter[values[attr].slug] = []
                })
            }
        },
        async change() {
        ...
        async clearFilter() {
            Object.keys(this.getAttributes).forEach((attr) => {
                this.filter[this.getAttributes[attr].slug] = []
            })
            this.setSearch(null)
            this.setFilter({})
            await this.getProducts()
