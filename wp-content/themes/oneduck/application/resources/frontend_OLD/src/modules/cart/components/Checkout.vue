<template>
    <div class="order__checkout">
        <template v-if="isAuth && isCartValid && cartTotal">
            ...
            <form class="order__info" :class="{ edit: isEditedAddress && form.isDelivery }">
                <div class="order__info__delivery" :class="{ show: form.isDelivery }">
                    ...
                    <div v-if="!isValidAddress && isEditedAddress && form.isDelivery" class="error_address">Минимальная длина адреса - 10 символов</div>
                </div>
        ...
        <template v-else>
            ...
            <template v-if="!isCartValid">
                В корзине присутствуют товары с некорректной кратностью или в количестве, превышающее доступное. Проверьте список и попробуйте еще раз.
            </template>
        </template>
    </div>
</template>
