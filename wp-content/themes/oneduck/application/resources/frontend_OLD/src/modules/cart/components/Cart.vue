<template>
    <div>
        <div class="basket__notice">
            Обратите внимание, в корзине вы видите цены для способа оплаты <b>{{ currentGroup }}</b>. Для отображения цен для способа оплаты <b>{{ alternativeGroup }}</b> переключите формат цен в шапке сайта
        </div>
...
export default {
    ...
    computed: {
        ...
        currentGroup() {
            return (this.userGroupMixed === 'cash') ? 'Н' : 'Б'
        },
        alternativeGroup() {
            return (this.userGroupMixed === 'cash') ? 'Б' : 'Н'
        }
    },
    methods: {
        ...mapActions({
